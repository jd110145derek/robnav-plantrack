...
# Preset
control_type = None
message = input(" 0: Pure_pursuit / 1: Stanley : " )
if message == "0" :
    control_type = 0
elif message == "1" :
    control_type = 1
    ...
    print("error")
message = input(" 0: Astar / 1: RRT Star : " )
if message == "0" :
    plan_type = 0
elif message == "1" :
    plan_type = 1
...
def main():
    ...
    while(True):
        ...
        if nav_pos is not None:
            if path is None:
                if plan_type == 0:
                    path = planner.planning(start=(int(car.x),int(car.y)), goal=nav_pos, inter= 20 , img=None)
                elif plan_type == 1:
                    path = planner.planning(start=(int(car.x),int(car.y)), goal=nav_pos, extend_lens = 10  ,img=None)            
                ...
                #controller._search_nearest(pos = (int(car.x),int(car.y)))
                if control_type == 0:
                    # PID Longitude Control
                    end_dist = np.hypot(path[-1,0]-car.x, path[-1,1]-car.y)
                    target_v = 10 if end_dist > 25 else 0

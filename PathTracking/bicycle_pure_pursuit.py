...
class PurePursuitControl:
    ...
    def feedback(self, state):
        ...
        import math
        while (min_dist**0.5 < ld):
            min_idx += 1
            min_dist = (x - self.path[min_idx,0])**2 + (y - self.path[min_idx,1])**2

        target = self.path[min_idx]
        
        a = math.atan2(y-target[1],x-target[0])
        alpha = a - np.deg2rad(yaw)
        next_delta = -np.rad2deg(math.atan2((2*l*np.sin(alpha)),ld))
        # print("a:",a)
        # print("alpha:",alpha)
        # print("w:",next_w)
        #####################################################################
